import{r as a,a as x}from"./assets/renderMarkup-c61de964.js";import{a as S,S as E,N as C}from"./assets/vendor-dcaf4cbb.js";const T="https://books-backend.p.goit.global/books/";async function h(e,t){try{return(await S.get(T+e,{params:t})).data}catch(o){console.error(o)}}console.log("category-card");const B=async e=>{a.categoryCardElem.innerHTML="",a.topSellerSection.innerHTML="";try{const t=await h("category",e),o=a.categoryCardElem.querySelector(".category-card-title");o&&o.remove();const s=t[0].list_name;document.querySelectorAll(".category-card-title").forEach(n=>{n.remove()}),a.categoryCardElem.insertAdjacentHTML("afterbegin",I(s)),x(M,a.categoryCardElem,t)}catch(t){console.error("Error fetching category books:",t)}};function I(e){return`
    ...
    `}function m(e="show"){const t=document.querySelector(".loader");e==="show"?t.style.display="inline-block":t.style.display="none"}const L=document.querySelector(".seller-section"),d=document.createElement("ul");d.classList.add("sellers-list");d.innerHTML='<span class="loader"></span>';L.innerHTML=`
  <h1 class="section-title">
    Best Sellers <span class="accent">Books</span>
  </h1>`;L.appendChild(d);const v=async()=>{m();try{const e=await h("top-books"),t=$(e,4);q(t);const o=document.querySelectorAll(".sellers-item");o.forEach(n=>{n.classList.add("animation-items")});const s=setTimeout(()=>{o.forEach(n=>{n.classList.remove("animation-items")})},500)}catch(e){console.error(e)}};v();function $(e,t){const o=[];for(;o.length<t;){const s=Math.floor(Math.random()*e.length);o.includes(s)||o.push(s)}return o.map(s=>e[s])}function q(e){const t=A(e);d.innerHTML="",d.appendChild(t)}function A(e){const t=document.createDocumentFragment();return e.forEach(({list_name:o,books:s})=>{const n=document.createElement("li");n.className="sellers-category",n.innerHTML=`

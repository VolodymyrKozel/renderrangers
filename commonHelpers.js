import{r as y,a}from"./assets/renderMarkup-3fdc6bca.js";import{a as B,S as I,N as M}from"./assets/vendor-dcaf4cbb.js";const N="https://books-backend.p.goit.global/books/";async function p(e,t){try{return(await B.get(N+e,{params:t})).data}catch(s){console.error(s)}}function q(e){e.forEach(t=>{t.classList.add("animation-items")}),setTimeout(()=>{e.forEach(t=>{t.classList.remove("animation-items")})},500)}function O(e,{_id:t,book_image:s,title:n,author:o,description:r,buy_links:c}){return`
...
`}const x=document.querySelector("body"),m=document.querySelector(".backdrop");m.addEventListener("click",U);const S=[];let l=!0;async function H(e){const t=await p(e);_(t),document.addEventListener("keydown",E)}function _(e){localStorage.setItem("currentBook",JSON.stringify(e)),S.push(e),m.style.display="flex",x.style.overflow="hidden",m.innerHTML="",m.insertAdjacentHTML("afterbegin",O(l,e)),console.log(j(e._id,"cart"))}function j(e,t){let s=JSON.parse(localStorage.getItem(t));s===null&&(s=[],localStorage.setItem(t,JSON.stringify(s))),s.find(n=>n._id===e)&&(document.querySelector(".modal-list-button").textContent="remove from the shopping list",l=!1,console.log("same adres"))}function J(e){let t=JSON.parse(localStorage.getItem(e));localStorage.setItem(e,JSON.stringify(S,...t))}function D(e,t){const n=JSON.parse(localStorage.getItem(t)).filter(o=>o._id!==e);localStorage.setItem(t,JSON.stringify(n))}function U(e){e.target.id==="modal-list-button-id"&&(l&&(J("cart"),e.target.textContent="add to shopping list",l=!1),l||(D(e.target.dataset.id,"cart"),l=!0,e.target.textContent="remove from the shopping list")),e.target.tagName==="svg"&&g(),e.target.id==="mBackdrop"&&g(),e.target.nodeName}function E(e){e.key==="Escape"&&(g(),document.removeEventListener("keydown",E))}function g(){m.style.display="none",x.style.overflow="auto"}console.log("category-card");a.mainContainer.addEventListener("click",z);const C=async e=>{a.mainContainer.innerHTML="";try{const t=await p("category",e),s=t[0].list_name;a.mainContainer.insertAdjacentHTML("afterbegin",P(s,t));const n=document.querySelectorAll(".sellers-item");q(n)}catch(t){console.error("Error fetching category books:",t)}};function z(e){const t=e.target.closest(".sellers-item").dataset.id;H(t)}function P(e,t){const s=e.split(" "),n=s.pop();return s.push(`<span class="accent">${n}</span>`),`
    <section class="seller-section">
      <h1 class="section-title category-card-title">${s.join(" ")}</h1>
      <ul class="sellers-category-list sellers-exclude flex-wrap">${y(F,t)}</ul>
    </section>
  ...
  `}function R(e="show"){const t=document.querySelector(".loader");e==="show"?t.style.display="inline-block":t.style.display="none"}const $=async()=>{a.mainContainer.innerHTML=`
    ...
    `,t.appendChild(o)}),t}function K(e){let t="";const s=Q(),n=s==="desktop"?5:s==="tablet"?3:1;for(let o=0;o<n&&o<e.length;o++){const{book_image:r,title:c,author:i,_id:d}=e[o];t+=`

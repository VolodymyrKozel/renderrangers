import{r as y,a}from"./assets/renderMarkup-8be60694.js";import{a as T,S as B,N as I}from"./assets/vendor-dcaf4cbb.js";const M="https://books-backend.p.goit.global/books/";async function p(e,t){try{return(await T.get(M+e,{params:t})).data}catch(s){console.error(s)}}function N(e){e.forEach(t=>{t.classList.add("animation-items")}),setTimeout(()=>{e.forEach(t=>{t.classList.remove("animation-items")})},500)}function q(e,{_id:t,book_image:s,title:o,author:n,description:r,buy_links:c}){return`
...
`}const x=document.querySelector("body"),m=document.querySelector(".backdrop");m.addEventListener("click",D);const S=[];let l=!0;async function A(e){const t=await p(e);H(t),document.addEventListener("keydown",E)}function H(e){localStorage.setItem("currentBook",JSON.stringify(e)),S.push(e),m.style.display="flex",x.style.overflow="hidden",m.innerHTML="",m.insertAdjacentHTML("afterbegin",q(l,e)),console.log(_(e._id,"cart"))}function _(e,t){let s=JSON.parse(localStorage.getItem(t));s===null&&(s=[],localStorage.setItem(t,JSON.stringify(s))),s.find(o=>o._id===e)&&(document.querySelector(".modal-list-button").textContent="remove from the shopping list",l=!1,console.log("same adres"))}function j(e){let t=JSON.parse(localStorage.getItem(e));localStorage.setItem(e,JSON.stringify(S,...t))}function J(e,t){const o=JSON.parse(localStorage.getItem(t)).filter(n=>n._id!==e);localStorage.setItem(t,JSON.stringify(o))}function D(e){e.target.id==="modal-list-button-id"&&(l&&(j("cart"),e.target.textContent="add to shopping list",l=!1),l||(J(e.target.dataset.id,"cart"),l=!0,e.target.textContent="remove from the shopping list")),e.target.tagName==="svg"&&g(),e.target.id==="mBackdrop"&&g(),e.target.nodeName}function E(e){e.key==="Escape"&&(g(),document.removeEventListener("keydown",E))}function g(){m.style.display="none",x.style.overflow="auto"}console.log("category-card");a.mainContainer.addEventListener("click",U);const C=async e=>{a.mainContainer.innerHTML="";try{const t=await p("category",e),s=t[0].list_name;a.mainContainer.insertAdjacentHTML("afterbegin",z(s,t));const o=document.querySelectorAll(".sellers-item");N(o)}catch(t){console.error("Error fetching category books:",t)}};function U(e){const t=e.target.closest(".sellers-item").dataset.id;A(t)}function z(e,t){const s=e.split(" "),o=s.pop();return s.push(`<span class="accent">${o}</span>`),`
  ...
  `}function F(e="show"){const t=document.querySelector(".loader");e==="show"?t.style.display="inline-block":t.style.display="none"}const $=async()=>{a.mainContainer.innerHTML=`
    ...
    </section>`;const e=document.createElement("ul");e.classList.add("sellers-list"),e.innerHTML='<span class="loader"></span>',a.mainContainer.appendChild(e),a.mainContainer.querySelector(".loader"),F();try{const t=await p("top-books"),s=G(t,4);R(e,s);const o=document.querySelectorAll(".sellers-item");o.forEach(r=>{r.classList.add("animation-items")});const n=setTimeout(()=>{o.forEach(r=>{r.classList.remove("animation-items")})},500);e.addEventListener("click",W)}catch(t){console.error(t)}};function R(e,t){const s=V(t);e.innerHTML="",e.appendChild(s)}$();async function W(e){if(e.preventDefault(),e.target.nodeName!=="BUTTON")return;const t=e.target,{name:s}=t.dataset;await C({category:s})}function G(e,t){const s=[];for(;s.length<t;){const o=Math.floor(Math.random()*e.length);s.includes(o)||s.push(o)}return s.map(o=>e[o])}function V(e){const t=document.createDocumentFragment();return e.forEach(({list_name:s,books:o})=>{const n=document.createElement("li");n.className="sellers-category",n.innerHTML=`
    ...
    `,t.appendChild(n)}),t}function Z(e){let t="";const s=K(),o=s==="desktop"?5:s==="tablet"?3:1;for(let n=0;n<o&&n<e.length;n++){const{book_image:r,title:c,author:i,_id:d}=e[n];t+=`

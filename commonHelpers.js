...
`}const S=document.querySelector("body"),m=document.querySelector(".backdrop");m.addEventListener("click",D);const x=[];let u=!0;async function A(e){const t=await p(e);H(t),document.addEventListener("keydown",E)}function H(e){localStorage.setItem("currentBook",JSON.stringify(e)),x.push(e),m.style.display="flex",S.style.overflow="hidden",m.innerHTML="",m.insertAdjacentHTML("afterbegin",q(e)),console.log(_(e._id,"cart"))}function _(e,t){let s=JSON.parse(localStorage.getItem(t));s===null&&(s=[],localStorage.setItem(t,JSON.stringify(s))),s.find(o=>o._id===e)&&(document.querySelector(".modal-list-button").textContent="remove from the shopping list",u=!1,console.log("same adres"))}function j(e){let t=JSON.parse(localStorage.getItem(e));localStorage.setItem(e,JSON.stringify(x,...t))}function J(e,t){const o=JSON.parse(localStorage.getItem(t)).filter(n=>n._id!==e);localStorage.setItem(t,JSON.stringify(o))}function D(e){e.target.id==="modal-list-button-id"&&(u&&(j("cart"),e.target.textContent="add to shopping list",u=!1),u||(J(e.target.dataset.id,"cart"),u=!0,e.target.textContent="remove from the shopping list")),e.target.tagName==="svg"&&g(),e.target.id==="mBackdrop"&&g(),e.target.nodeName}function E(e){e.key==="Escape"&&(g(),document.removeEventListener("keydown",E))}function g(){m.style.display="none",S.style.overflow="auto"}console.log("category-card");r.mainContainer.addEventListener("click",U);const C=async e=>{r.mainContainer.innerHTML="";try{const t=await p("category",e),s=t[0].list_name;r.mainContainer.insertAdjacentHTML("afterbegin",z(s,t));const o=document.querySelectorAll(".sellers-item");N(o)}catch(t){console.error("Error fetching category books:",t)}};function U(e){const t=e.target.closest(".sellers-item").dataset.id;A(t)}function z(e,t){const s=e.split(" "),o=s.pop();return s.push(`<span class="accent">${o}</span>`),`
  ...
  `}function F(e="show"){const t=document.querySelector(".loader");e==="show"?t.style.display="inline-block":t.style.display="none"}const $=async()=>{r.mainContainer.innerHTML=`
    ...
    `,t.appendChild(n)}),t}function Z(e){let t="";const s=K(),o=s==="desktop"?5:s==="tablet"?3:1;for(let n=0;n<o&&n<e.length;n++){const{book_image:c,title:a,author:i,_id:l}=e[n];t+=`
